...
async def create_cover_letter(
    ...
    """GMS를 사용한 자기소개서 항목 생성"""
    
    logger.info(f"GMS 자기소개서 항목 생성 시작: {content.content_number}")
...
답변은 반드시 자기소개서만 작성합니다. 자기소개서 이외 내용은 포함하면 안됩니다."""
    ...
    # GMS 에이전트 생성 및 실행
    cover_letter_agent = await gms_utils.get_gms_agent(
        name="GMS CoverLetter Draft Assistant",
        model="gpt-4.1",
    ...
    response = await Runner.run(cover_letter_agent, prompt)
    
    logger.info(f"GMS 자기소개서 초안 작성 완료: {response.final_output}")
    return response.final_output


async def create_cover_letter_all(request: CreateCoverLetterRequest) -> List[CoverLetterItem]:
    """GMS를 사용한 전체 자기소개서 생성"""
    
    logger.info(f"GMS 자기소개서 초안 작성 요청")
...
async def edit_cover_letter_service(request: EditCoverLetterRequest) -> str:
    """GMS를 사용한 자기소개서 수정 제안 서비스"""
    
    logger.info(f"GMS 자기소개서 수정 제안 시작")
    ...
    system_prompt = """당신은 '제트'라는 이름의 AI로, 전문적인 자기소개서 수정 도우미입니다. 기업과 직무 분석을 바탕으로 지원자의 경험과 프로젝트를 잘 활용하여 맞춤형 자기소개서 수정 방향을 제시해주세요. 직접적인 수정은 절대 하지 말고 수정 방향만 제안해주세요."""
    
    # GMS 에이전트 생성 및 실행
    edit_agent = await gms_utils.get_gms_agent(
        name="GMS CoverLetter Edit Assistant",
        model="gpt-4.1",
        instructions=system_prompt,
        tools=[],  # 빈 도구 목록 사용
        output_type=EditSuggestionList
    )
    
    response = await Runner.run(edit_agent, prompt)
    
    # 포맷팅된 결과 반환
    ai_message_str = await parse_edit_suggestion(response.final_output)
    
    logger.info(f"GMS 자기소개서 수정 제안 완료")
...
"""
    ...
    # GMS 에이전트 생성 및 실행
    chat_type_agent = await gms_utils.get_gms_agent(
        name="GMS Chat Type Classifier",
        model="gpt-4.1",
        instructions=system_prompt,
        tools=[],  # 빈 도구 목록 사용
        output_type=ChatTypeResponse
    )
    
    response = await Runner.run(chat_type_agent, user_message)
    
    chat_type = response.final_output.chat_type
    logger.info(f"chat_type: {chat_type}")
    return chat_type


async def chat_with_cover_letter_service(request: ChatCoverLetterRequest) -> dict:
    """GMS를 사용한 자기소개서 관련 채팅 기능을 제공하는 서비스"""
    
    try: 
        logger.info(f"GMS chat_with_cover_letter_service 호출")
        ...
        try:
            # GMS 에이전트 생성 및 실행
            chat_agent = await gms_utils.get_gms_agent(
                name="GMS CoverLetter Chat Assistant",
                model="gpt-4.1",
                instructions=system_prompt_chat,
                tools=[],  # 빈 도구 목록 사용
            )
            
            response = await Runner.run(chat_agent, request.user_message)
        ...
        logger.error(f"GMS cover_letter_service 오류: {e}")
        return {
            "status": "error",
            "content": f"GMS cover_letter_service 오류: {e}"
        }
